...
By default, SCT segmentations are returned as one source-geometry 2D image per input image. These outputs set `Keep_image_geometry = 1`, patch returned-series identity into Meta and `IceMiniHead`, and restamp scanner storage counters in the same style as returned originals. If `segmentpostprocessing` is disabled, SCT segmentations are packed into one explicit 3D MRD volume with `Keep_image_geometry = 0`, `matrix_size[2] = N`, `slice_count = NumberOfSlices = ImagesInAcquisition = N`, and the source `IceMiniHead` removed.
