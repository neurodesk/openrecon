...
The scanner GUI is limited to 14 parameters. Advanced pTPI defaults remain in
the wrapper: coil compression is enabled, echo normalization is `none`, and
temporal SVD denoising is enabled. Machine-level limits default to all coils
and 8 workers through `SODIUMGRIDDING_PTPI_MAX_COILS` and
`SODIUMGRIDDING_PTPI_MAX_WORKERS`, respectively.
