...
| `segmentationcolormap` | boolean | `false` | Adds the scanner segmentation colourmap metadata to returned SCT segmentations. |
| `sctdebugthresholdsegment` | boolean | `false` | Skips SCT inference and returns a simple threshold segmentation for testing the OpenRecon send path. |
...
The app returns the original source images first when `sendoriginal` is enabled, then one derived MRD image series for a single selected analysis, or one derived series per SCT output for multiclass analyses and combined modes. Multiclass outputs are split into separate MRD series for cord, gray matter, white matter, and lesion labels as generated by SCT. The series description is based on the input series description plus the output id, for example `SOURCE_sct_deepseg_spinalcord`.

The `sct_spinalcord_area` mode returns the spinal cord segmentation as its
derived image series, then returns a separate `sct_spinalcord_area_metrics`
DICOM image series with the area summary and per-level table burned into a
one-page report. It runs `sct_label_vertebrae` first, then gives the generated
disc labels to `sct_process_segmentation` so cross-sectional area can be
calculated per vertebral level. It writes the aggregate mean, per-level
`MEAN(area)` values, detected vertebral levels, and generated vertebral/disc
...
lesion mask series receives DICOM-facing lesion-analysis metadata fields,
including lesion count, total lesion volume, maximum lesion length, maximum
axial damage ratio, and tissue bridge widths. The app also returns a separate
`sct_lesion_analysis_metrics` DICOM image series with the lesion-analysis
summary and lesion table burned into a one-page report.

SCT segmentations are returned as one source-geometry 2D image per input image, not as detached explicit 3D volumes. These outputs set `Keep_image_geometry = 1` and `SegmentSourceGeometry = 1`, patch returned-series identity into Meta and `IceMiniHead`, and restamp scanner storage counters in the same style as returned originals.

When `sctdebugthresholdsegment` is enabled, the wrapper skips SCT model execution and writes threshold-derived debug NIfTI outputs into the normal SCT output slots. The resulting images still go through the same MRD conversion, series allocation, scanner metadata stamping, and send validation path as real SCT outputs.
