...
def get_fire_workflow_base(package_name):
    ...
    return f'wip_070_fire_IceFireImageAddin_{package_name}'
...
def create_fire_ini_template(fire_img_name, startup_script_path, fire_search_string):
    return textwrap.dedent(
        f'''\
        ; FIRE chroot configuration generated by neurorecon.
...
def create_fire_install_text(fire_img_name, fire_ini_name):
    return textwrap.dedent(
        ...
        This folder is laid out relative to `%CustomerIceProgs%`, typically `...\\MriCustomer\\Ice`.
        Copy or merge the contents of this folder into that directory.

        Included scanner-relative paths
        -------------------------------
        - `wip_070_fire_*.ipr`
        - `wip_070_fire_*.xml`
        - `fire\\chroot\\{fire_img_name}`
        - `fire\\{fire_ini_name}`
        - `fire\\config\\wip_070_fire_*.json`
        - `fire\\share\\code`, `fire\\share\\dependency`, `fire\\share\\log`
        - The included PDF is the recipe/operator documentation.

        If the bundle includes additional `fire\\*.ini` files, copy them to `%CustomerIceProgs%\\fire\\`
        only when your workflow XML references those specific files.
...
def build_fire_bundle_stage(stage_dir, fire_img_path, fire_ini_name, fire_ini_text, install_text, docs_source_path, json_data, package_name, recipe_dir=None):
    ...
    copy_optional_fire_ini_files(stage_dir, recipe_dir)
    workflow_base = get_fire_workflow_base(package_name)
    default_config_id = get_default_openrecon_config_id(json_data)
    (stage_dir / f'{workflow_base}.ipr').write_text(create_fire_ipr_text())
    (stage_dir / f'{workflow_base}.xml').write_text(create_fire_workflow_xml_text(default_config_id, fire_ini_name))
    write_fire_config_json_files(stage_dir, recipe_dir, json_data)
